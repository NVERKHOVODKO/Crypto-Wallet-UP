...
public class UserController : ControllerBase
{
    ...
    {
        return await _dbRepository.Get<User>(x => x.Login == nickname && x.Id != currentUserId).FirstOrDefaultAsync() ==
               null;
    }
    ...
    public async Task<ActionResult> ChangeUserName(Guid id, string newLogin)
    {
        ...
        if (existingUser == null)
            throw new EntityNotFoundException("User not found");
        if (newLogin.Length == 0) throw new IncorrectDataException("Заполните данные");
        if (newLogin.Length > 32) throw new IncorrectDataException("Пароль должен быть короче 32 символов");
        if (newLogin.Length < 4) throw new IncorrectDataException("Пароль должен быть длиннее 4 символов");
    ...
    public async Task<ActionResult> ChangePassword([FromBody] ChangePasswordRequest request)
    {
        ...
        if (request.Password.Length == 0) return UnprocessableEntity("Заполните данные");
        if (request.Password.Length > 32) return UnprocessableEntity("Пароль должен быть короче 32 символов");
        if (request.Password.Length < 4) return UnprocessableEntity("Пароль должен быть длиннее 4 символов");
        existingUser.Password = _hashHelpers.HashPassword(request.Password, existingUser.Salt);
        existingUser.DateUpdated = DateTime.UtcNow;
