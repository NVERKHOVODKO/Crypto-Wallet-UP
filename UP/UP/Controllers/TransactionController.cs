...
namespace UP.Controllers
{
    ...
    public class TransactionController: ControllerBase
    {
        ...
        public TransactionController(IDbRepository dbRepository,ILogger<TransactionController> logger, IUserRepository userRepository, ICurrencyRepository currencyRepository, ITransactionsRepository transactionsRepository)
        {
            _logger = logger;
        ...
        [HttpGet, Route("getCoinQuantity/{coinName}/{quantityUSD}")]
        ...
        public async Task<ActionResult> Convert([FromBody] ConvertRequest request)
        {
            ...
            double startCoinQuantityInUserWallet = _userRepository.GetCoinQuantityInUserWallet(request.UserId, request.ShortNameStart);
            ...
            var conversionData = new ModelsEF.Conversion
            {
                Id = Guid.Empty,
                Commission = 0,
        ...
        public async Task<ActionResult> BuyCrypto([FromBody] BuyCryptoRequest request)
        {
            ...
            double quantityUSDTInUserWallet = _userRepository.GetCoinQuantityInUserWallet(request.UserId, "usdt");
            ...
            double coinQuantity = await _currencyRepository.GetCoinQuantity(request.Quantity, request.CoinName);
        ...
        public async Task<ActionResult> SellCrypto([FromBody] SellCryptoRequest request)
        {
            ...
            double quantityInUserWallet = _userRepository.GetCoinQuantityInUserWallet(request.UserId, request.CoinName);
            ...
            _currencyRepository.AddCryptoToUserWallet(request.UserId, "usdt", await _currencyRepository.GetCoinPrice(request.QuantityForSell, request.CoinName));
            return Ok("Транзакция совершена успешно");
        }
        ...
        public async Task<ActionResult> SendCrypto([FromBody] SendCryptoRequest request)
        {
            ...
            double quantityInUserWallet = _userRepository.GetCoinQuantityInUserWallet(request.SenderId, request.CoinName);
            ...
            _currencyRepository.WriteTransactionToDatabase(request.CoinName, request.QuantityForSend, request.SenderId, request.ReceiverId);
        ...
        public async Task<ActionResult> WithdrawUSDT([FromBody] DTO.WithdrawRequest request)
        {
            ...
            double quantityInUserWallet = _userRepository.GetCoinQuantityInUserWallet(request.UserId, "usdt");
            ...
            _currencyRepository.WriteWithdrawToDatabase(request.QuantityForWithdraw, (request.QuantityForWithdraw * 0.02), request.UserId);
            return Ok("Транзакция выполнена успешно");
        }
        
        [HttpGet, Route("getUserWithdrawalsHistory/{userId}")]
        ...
        [HttpGet, Route("getUserTransactionsHistory/{userId}")]
